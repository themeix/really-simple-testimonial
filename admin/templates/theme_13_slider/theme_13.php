...
if ($rst_testimonial_themes == 13) {


    wp_enqueue_script('rst_theme_13_script_1', plugin_dir_url(__FILE__) . 'assets/js/slick.min.js', array('jquery'), time(), true);
    wp_enqueue_script('rst_theme_13_script_2', plugin_dir_url(__FILE__) . 'assets/js/app.js', array('jquery'), time(), true);


    wp_enqueue_style('rst_theme_13_style_1', plugin_dir_url(__FILE__) . 'assets/css/slick.css', array(), time(), 'all');
    wp_enqueue_style('rst_theme_13_style_2', plugin_dir_url(__FILE__) . 'assets/css/rst-style-8.css', array(), time(), 'all');
    ...
    $rst_query_theme_13 = new WP_Query($args);

    if ($rst_query_theme_13->have_posts()) {
        ?>
        ...
        <div class="rst-container pt-28">
            <div class="rst-testimonial-box-8 relative">
                <div class="rst-slider">

                    <?php
                    while ($rst_query_theme_13->have_posts()) {
                        $rst_query_theme_13->the_post();
                        ...
                        <div class="rst-item p-6 border rounded-md">
                            <div class="rst-author-box text-center">
                                <div class="rst-author-img inline-block">
                                    <img class="h-15 w-15 rounded-full "
                                         src="<?php echo $rst_author_image2[0]; ?>"
                                         alt="author">

                                </div>
                                <p class="rst-author-text text-base font-normal text-coolGray-600 mt-4">
                                    <?php if (!empty($rst_content)) {
                                        echo $rst_content;
                                    } ?>
                                </p>
                                <h4 class="author-name text-xl font-bold text-coolGray-900 mt-4">
                                    <?php if (!empty($rst_author_name)) echo $rst_author_name; ?>
                                </h4>
                                <p class="rst-author-title text-coolGray-600  text-base">
                                    <?php if (!empty($rst_author_position)) echo $rst_author_position; ?>
                                </p>
                                <div class="rst-author-rating   mt-4">
                                    <ul>
