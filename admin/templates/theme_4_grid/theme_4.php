...
if ($rst_testimonial_themes == '4') {
    ...
    wp_enqueue_style('rst_theme_4_style_1', plugin_dir_url(__FILE__) . 'assets/css/rst-style-4.css', array(), time(), 'all');
    ...
    if ($rst_query_theme_4->have_posts()) {
        ...
        <style type="text/css">
            ...
            .rst_<?php echo esc_attr( $postid );?> .rst_testimonial_item {
                ...
                background-color: <?php echo esc_attr($rst_item_bg_color); ?> !important;
                padding: <?php echo esc_attr($rst_item_padding); ?>px !important;
                border-radius: <?php echo esc_attr($rst_item_border_radius); ?> !important;
            }

            .rst_<?php echo esc_attr( $postid );?> .rst-border-coolGray-100 {
                border-color: <?php echo esc_attr($rst_item_border_color); ?> !important;
            }

            .rst_<?php echo esc_attr( $postid );?> .rst-pur-css-arrow {
                border-width: 12px 12px 0 12px;
                border-color: <?php echo esc_attr($rst_item_border_color); ?> transparent transparent transparent !important;
            }
        ...
        <div class="rst-box-2 rst_container_box_thm_4 rst_<?php echo esc_attr($postid); ?>">
            <div class="rst-container  rst-pt-20">

                <div class="sm:rst-grid rst-flex rst-flex-col sm:rst-grid-cols-2  lg:rst-grid-cols-3 rst-gap-6">
                    <?php
                    while ($rst_query_theme_4->have_posts()) {
                        ...
                        <div class="rst-item rst-text-center rst_testimonial_item">
                            <div class="rst-item-wrap rst-border rst-p-4  rst-rounded-md rst-border-coolGray-100 hover:rst-rst-border-coolGray-200 rst-transition">
                                <div class="rst-author-images rst-inline-block">
                                    <?php if(!empty($rst_author_image)){ ?>
                                    <img class="rst-h-15 rst-w-15 rst-rounded-full rst_author_image"
                                         src="<?php echo esc_url($rst_author_image[0]); ?>" alt="images">
                                    <?php } ?>
                                </div>
                                <p class="rst-text rst-font-normal rst-text-base  rst-mt-4 rst_content">
                                    <?php if (!empty($rst_content)) {
                                        echo esc_attr($rst_content);
                                    } ?>
                                </p>
                                <div class="rst-author-rating rst-flex rst-justify-center  rst-mt-4 rst_rating">
                            ...
                            <div class="rst-pur-css-arrow rst-w-0 rst-h-0 rst-rst-border-solid rst-relative rst-m-auto"></div>
                            <h5 class="rst-author-name rst-text-xl   rst-mt-4 rst_author_name">
                                <?php if (!empty($rst_author_name)) echo esc_attr($rst_author_name); ?>
                            </h5>
                            <h6 class="rst-author-title rst-text-base rst-text-coolGray-600  rst-opacity-60">
